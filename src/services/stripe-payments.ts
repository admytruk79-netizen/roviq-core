...
export async function createStripePaymentIntent(principal:Principal,input:{
  ...
  try{
    ...
    await updatePaymentState(principal,local.id,'failed',{payload:{provider:'stripe',stage:'create',error:error instanceof Error?error.message:String(error)}});
  ...
  const updated=await pool.query(
    `update payment_intents
      ...
      returning *`,
    [local.id,providerIntentId,JSON.stringify({stripeStatus:json.status??null})]
  );
  if(!updated.rowCount){
