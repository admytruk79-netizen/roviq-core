...
export async function updateTransportStatus(principal: Principal, dispatchId:string, status:TransportStatus, metadata:Record<string,unknown> = {}) {
  ...
  try {
    ...
    let updated = await client.query(`update transport_dispatches set status=$1,metadata=metadata || $2::jsonb,updated_at=now() ${ts} where id=$3 returning *`,[status,JSON.stringify(metadata),dispatchId]);
    ...
    // transitionCase manages its own connection/transaction and can't run inside this one; it must
    // follow the commit above (matches the pre-existing behavior: the case transition was never
    // atomic with the dispatch update, only ever ordered after it).
