...
export async function syncInventoryFeed(sourceKey:string,vehicles:InventoryFeedVehicle[],marginCents=0,completeSnapshot=false){
  ...
  try{
    ...
    for(const v of valid){
      ...
      await client.query(`
        ...
        [sourceKey,v.id,v.vin??null,v.year??null,v.make,v.model,v.trim??null,v.mileage??null,v.exteriorColor??null,v.drivetrain??null,v.fuelType??null,v.bodyStyle??null,JSON.stringify(v.images??[]),v.priceCents??null,v.marginCents??marginCents,v.dealerName??null,v.dealerUrl??null,JSON.stringify(v.raw??{})]);
    }
