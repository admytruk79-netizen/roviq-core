...
import { createDeadline, createServiceCase, getCaseTimeline, publishCaseIntegrationEventSafely, raiseException, transitionCase, withIdempotency } from '../../services/orchestration.js';
...
import { getExceptionQueue } from '../../services/exception-engine.js';
...
export async function caseRoutes(app:FastifyInstance){
 app.post('/api/maintenance/cases',{preHandler:requireRole('customer','admin')},async(req,reply)=>{const body=createCaseBody.parse(req.body);if(req.principal.role==='customer'&&(body.selectionMode==='ops_override'||body.selectionMode==='auto_dispatch'||body.selectionMode==='dealer_controlled'||body.originatingActorId||body.relationshipOwnerActorId))return reply.code(403).send({error:'case_authority_fields_admin_only'});const key=typeof req.headers['idempotency-key']==='string'?req.headers['idempotency-key']:undefined;const result=await withIdempotency(req.principal,key,'create_service_case',body,async(client)=>({status:201,body:{case:await createServiceCase(req.principal,body,client)}}));if(result.fresh&&result.status===201){const created=(result.body as {case:{id:string;state:string;priority:string}}).case;await publishCaseIntegrationEventSafely(created.id,'CASE_CREATED',req.principal,{state:created.state,priority:created.priority});}return reply.code(result.status).send(result.body);});
 app.get('/api/customers/me/cases',{preHandler:requireRole('customer')},async(req)=>{const r=await pool.query(`select c.id,c.state,c.priority,c.drivability,c.case_type,c.current_owner_role,c.recommended_actor_id,c.selected_actor_id,c.selection_mode,c.created_at,c.updated_at,c.completed_at,c.cancelled_at,s.customer_status,s.customer_message from service_cases c left join case_snapshots s on s.case_id=c.id where c.customer_actor_id=$1 order by c.created_at desc limit 200`,[req.principal.actorId]);return{cases:r.rows};});
