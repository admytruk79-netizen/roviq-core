...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post('/api/admin/testing/customer-session', { preHandler: requireRole('admin') }, async (req, reply) => {
    ...
    try {
      ...
      if (!domain.rowCount) { await client.query('rollback'); return reply.code(500).send({ error:'maintenance_domain_missing' }); }
      let actor = await client.query(`select id from actors where actor_type='customer' and status='active' and attributes->>'testContext'='admin_customer_portal' order by created_at asc limit 1`);
      if (!actor.rowCount) actor = await client.query(`insert into actors(domain_id,actor_type,status,attributes) values($1,'customer','active',$2) returning id`,[domain.rows[0].id,JSON.stringify({ testContext:'admin_customer_portal', displayName:'ROVIQ Admin Test Customer' })]);
      const actorId = actor.rows[0].id as string;
      const recovered = await client.query(`update service_cases c set customer_actor_id=$1, updated_at=now() where c.customer_actor_id is null and exists (select 1 from audit_log a where a.object_type='service_case' and a.object_id=c.id and a.action='create_case' and a.principal_role='admin' and a.principal_actor_id is null) returning c.id`,[actorId]);
