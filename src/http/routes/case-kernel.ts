...
export async function caseKernelRoutes(app:FastifyInstance){
  app.post('/api/core/cases',{preHandler:requireRole('customer','admin','partner','diagnostic','tow','parts','fleet')},async(req,reply)=>{
    ...
    const result=await withIdempotency(req.principal,key,'core.case.create',body,async(client)=>{
      ...
      const ownerActorId=req.principal.role==='customer'?null:req.principal.actorId??null;
      const r=await client.query(`insert into core_cases(case_type,market_id,location_id,customer_actor_id,current_owner_actor_id,priority,requirements,constraints,attributes)
        values($1,$2,$3,$4,$5,$6,$7,$8,$9) returning *`,[body.caseType,body.marketId??null,body.locationId??null,customerActorId,ownerActorId,body.priority,body.requirements,body.constraints,body.attributes]);
  ...
  app.post('/api/core/cases/:id/assign',{preHandler:requireRole('admin')},async(req,reply)=>{
    const {id}=req.params as {id:string};
    const body=z.object({actorId:z.string().uuid(),expectedVersion:z.number().int().positive()}).parse(req.body);
    const key=typeof req.headers['idempotency-key']==='string'?req.headers['idempotency-key']:undefined;
    if(!key)return reply.code(400).send({error:'idempotency_key_required'});
    try{
      const result=await withIdempotency(req.principal,key,`core.case.assign:${id}`,body,async(client)=>{
        if(!client)throw new Error('transaction_required');
        const existing=await loadCoreCaseForPrincipal(req.principal,id,client);
        if(!existing)return {status:404,body:{error:'case_not_found'} as Record<string,unknown>};
        const q=await client.query('select * from core_cases where id=$1 for update',[id]);
        const locked=q.rows[0];
        if(Number(locked.version)!==body.expectedVersion)return {status:409,body:{error:'version_conflict',currentVersion:Number(locked.version)}};
        const actor=await client.query('select id,status from actors where id=$1',[body.actorId]);
        if(!actor.rowCount)return {status:404,body:{error:'actor_not_found'}};
        if(actor.rows[0].status!=='active')return {status:409,body:{error:'actor_not_active'}};
        const policy=await evaluateCorePolicy({action:'case.assign',principal:req.principal,caseRecord:locked,facts:{assignment:{actorId:body.actorId}}},client);
        if(policy.decision==='deny')return {status:403,body:{error:'policy_denied',reason:policy.reason}};
        if(policy.decision==='require_review')return {status:409,body:{error:'policy_review_required',reason:policy.reason}};
        const nextVersion=Number(locked.version)+1,correlationId=randomUUID();
        const u=await client.query('update core_cases set current_owner_actor_id=$2,version=$3,updated_at=now() where id=$1 and version=$4 returning *',[id,body.actorId,nextVersion,body.expectedVersion]);
        if(!u.rowCount)return {status:409,body:{error:'version_conflict'}};
        const payload={fromActorId:locked.current_owner_actor_id??null,toActorId:body.actorId};
        await client.query(`insert into core_case_events(case_id,actor_id,event_type,correlation_id,payload,payload_hash,previous_version,new_version)
          values($1,$2,'CASE_ASSIGNED',$3,$4,$5,$6,$7)`,[id,req.principal.actorId??null,correlationId,payload,hash(payload),body.expectedVersion,nextVersion]);
        await client.query(`insert into core_outbox(aggregate_type,aggregate_id,event_type,payload,correlation_id)
          values('case',$1,'CASE_ASSIGNED',$2,$3)`,[id,{caseId:id,...payload,version:nextVersion},correlationId]);
