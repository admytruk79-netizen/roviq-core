...
async function setupDeferredBookingRace(){
  ...
  const serviceCase=await pool.query(`insert into service_cases(domain_id,case_type,state,customer_actor_id,selected_actor_id,current_owner_role,current_owner_actor_id)
    values($1,'maintenance','provider_pending',$2,$3,'partner',$3) returning id`,[
