...
describe('portal access acceptance matrix', () => {
  ...
  beforeAll(async () => {
    if (!ADMIN_EMAIL || !ADMIN_PASSWORD) throw new Error('bootstrap_admin_test_credentials_missing');

    app = await buildApp();

    const existingAdmin = await pool.query(
      `select id from principal_identities where lower(email)=lower($1) limit 1`,
      [ADMIN_EMAIL]
    );
    if (!existingAdmin.rowCount) {
      ...
      await pool.query(
        `insert into principal_identities(actor_id,email,role,password_salt,password_hash,active)
         values(null,lower($1),'admin',$2,$3,true)`,
        [ADMIN_EMAIL,salt,hash]
      );
    }
