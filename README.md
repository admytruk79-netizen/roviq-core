...
`npm test` (used by CI) only runs fast unit tests with no external dependencies — `tests/isolation.test.ts` and `tests/case-access.test.ts`. Six suites drive a real Fastify app instance end-to-end:
...
2. Functional test coverage for the remaining domains (triage and integrations currently have no dedicated tests — maintenance, transport, mobility, parts, payments and notifications are covered end-to-end, case-access/isolation are covered by unit tests)
