...
`npm test` (used by CI) only runs fast unit tests with no external dependencies — `tests/isolation.test.ts` and `tests/case-access.test.ts`. Five suites drive a real Fastify app instance end-to-end:
...
2. Functional test coverage for the remaining domains (notifications, triage, integrations currently have no dedicated tests — maintenance, transport, mobility, parts and payments are covered end-to-end, case-access/isolation are covered by unit tests)
