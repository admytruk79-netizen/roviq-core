...
`npm test` (used by CI) only runs fast unit tests with no external dependencies — `tests/isolation.test.ts` and `tests/case-access.test.ts`. Seven suites drive a real Fastify app instance end-to-end:
...
2. Functional test coverage for the remaining domain (integrations currently has no dedicated tests — maintenance, transport, mobility, parts, payments, notifications and triage are covered end-to-end, case-access/isolation are covered by unit tests)
