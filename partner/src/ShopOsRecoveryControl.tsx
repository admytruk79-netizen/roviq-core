...
type Resource={id:string;resource_type:string;display_name:string;operational_state?:'available'|'busy'|'blocked'|'offline';active?:boolean;connection_status?:string};
...
export function ShopOsRecoveryControl(){
  ...
  const availableResources=useMemo(()=>resources.filter(r=>r.active!==false&&r.connection_status==='active'&&!['blocked','offline'].includes(r.operational_state??'available')),[resources]);
