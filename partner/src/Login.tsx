  ...
  return (
    <div className="shell grid-glow grid min-h-screen lg:grid-cols-[1.05fr_.95fr]">
      ...
      <section className="flex min-h-screen items-center justify-center px-5 py-10 sm:px-8"><form onSubmit={submit} className="panel w-full max-w-md p-6 sm:p-8">
        ...
        {error && <p role="alert" aria-live="assertive" className="mt-4 rounded-lg border border-red-400/20 bg-red-500/10 px-3 py-2 text-sm text-red-300">{error}</p>}<button className="primary mt-6 w-full" type="submit" disabled={loading} aria-busy={loading}>{loading ? 'Signing in…' : 'Enter partner portal'}</button>
      </form></section>
    </div>
  );
}
