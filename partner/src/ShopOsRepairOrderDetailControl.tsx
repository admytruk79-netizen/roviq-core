...
export function ShopOsRepairOrderDetailControl(){
  ...
  const loadOrders=useCallback(async()=>{
    ...
    setSelectedId(current=>current&&response.repairOrders.some(order=>order.id===current)?current:(response.repairOrders[0]?.id??null));
  },[]);
  ...
  const loadDetail=useCallback(async(id:string)=>{
    ...
    try{
      ...
      setDetail(response);
    }catch(e){if(requestId===requestSequence.current)setError(`Repair order could not load. ${human(e instanceof Error?e.message:'request failed')}. Refresh and try again.`)}finally{if(requestId===requestSequence.current)setLoading(false)}
  },[]);

  useEffect(()=>{setLoading(true);void loadOrders().catch(e=>setError(`Repair orders could not load. ${human(e instanceof Error?e.message:'request failed')}`)).finally(()=>setLoading(false))},[loadOrders]);
