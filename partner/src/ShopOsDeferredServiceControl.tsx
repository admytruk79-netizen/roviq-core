...
function range(){const from=new Date();from.setDate(from.getDate()-1);const to=new Date();to.setDate(to.getDate()+90);return{from:from.toISOString(),to:to.toISOString()}}
...
export function ShopOsDeferredServiceControl(){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const[d,b]=await Promise.all([
        ...
        api.get<Board>(`/api/shop-os/board?from=${encodeURIComponent(r.from)}&to=${encodeURIComponent(r.to)}`)
      ]);
      if(requestId!==requestSequence.current)return;
      setItems(d.deferredItems??[]);
  ...
  async function act(item:DeferredServiceItem,action:'remind'|'book'|'complete'|'dismiss'|'reopen'){
    ...
    if(action==='book'&&!appointmentId){setError('Choose the matching appointment before marking deferred service booked.');return;}
    setBusy(item.id);setMessage(null);setError(null);
    try{
      ...
      await load();
    }catch(e){setError(`Deferred service was not updated. ${human(e instanceof Error?e.message:'request failed')}. Refresh and try again.`)}finally{setBusy(null)}
  }
  ...
  return <section className="mt-8" aria-labelledby="deferred-service-heading">
    ...
    <div className="mt-5 space-y-3">{!loading&&ordered.length===0&&<div className="panel p-6"><p className="font-semibold">No deferred service to follow up</p><p className="muted mt-1 text-sm">Deferred recommendations will appear here automatically from repair-order decisions.</p></div>}{ordered.map(item=>{const choices=eligibleDeferredAppointments(item,appointments);const primary=deferredPrimaryAction(item.status);return <article key={item.id} className="panel p-4"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-white/10 bg-white/[.04] px-2.5 py-1 text-[11px] font-bold uppercase tracking-[.1em]">{human(item.status)}</span><span className="muted text-xs">{human(item.severity)}</span></div><h3 className="mt-2 font-bold">{item.line_description?.trim()||human(item.service_category)||'Deferred service'}</h3>{item.reason&&<p className="muted mt-1 text-sm">{item.reason}</p>}<div className="muted mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs"><span>Estimate {money(item.estimated_amount)}</span><span>Return target {when(item.target_return_at)}</span><span>Next follow-up {when(item.next_follow_up_at)}</span><span>{item.follow_up_count??0} reminders</span></div>{item.booked_appointment_id&&<p className="mt-2 text-xs text-emerald-100">Booked appointment: {item.booked_appointment_id}</p>}</div><div className="flex min-w-[17rem] flex-col gap-2">{(item.status==='open'||item.status==='reminded')&&<><label className="text-xs"><span className="muted">Matching scheduled appointment</span><select className="input mt-1 w-full" value={selectedAppointment[item.id]??''} onChange={e=>setSelectedAppointment(current=>({...current,[item.id]:e.target.value}))}><option value="">Choose appointment</option>{choices.map(a=><option key={a.id} value={a.id}>{when(a.starts_at)} · {human(a.service_category)}</option>)}</select></label>{choices.length===0&&<p className="muted text-xs">No matching held/confirmed appointment is available yet.</p>}<button className="primary" type="button" disabled={busy===item.id||choices.length===0} onClick={()=>void act(item,'book')}>Link scheduled return</button></>}{primary&&<button className="secondary" type="button" disabled={busy===item.id} onClick={()=>void act(item,primary)}>{primary==='remind'?'Send reminder':primary==='complete'?'Mark complete':'Reopen follow-up'}</button>}{['open','reminded','booked'].includes(item.status)&&<button className="secondary" type="button" disabled={busy===item.id} onClick={()=>void act(item,'dismiss')}>Dismiss follow-up</button>}{item.status==='completed'&&<p className="muted text-xs">Completed deferred work stays visible as service history.</p>}</div></div></article>})}</div>
  </section>;
}
