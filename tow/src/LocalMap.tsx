...
export function LocalMap({caseId,dispatchId,dispatchStatus}:{caseId?:string;dispatchId?:string;dispatchStatus?:string}){
  ...
  useEffect(()=>{if(!mapNode.current||mapRef.current)return;const center=live??point(spatial?.transport_location)??point(spatial?.origin)??point(spatial?.destination)??{lat:45.5231,lng:-122.6765};const map=new maplibregl.Map({container:mapNode.current,style:roviqLocalStyle(mode),center:[center.lng,center.lat],zoom:13,pitch:assigned?46:20,bearing:0,attributionControl:true,minZoom:3,maxZoom:18});mapRef.current=map;map.on('dragstart',()=>setFollowing(false));map.on('load',()=>{setError('');map.resize();void render(true)});map.on('styledata',()=>drawRoute(route));map.on('error',(e:any)=>{const m=String(e?.error?.message??'');if(m.includes('source')||m.includes('tile')||m.includes('network'))setError('Map tiles are temporarily unavailable.')});const resize=()=>map.resize();window.addEventListener('resize',resize);const timer=window.setTimeout(resize,250);return()=>{window.clearTimeout(timer);window.removeEventListener('resize',resize);markers.current.forEach(m=>m.remove());markers.current=[];map.remove();mapRef.current=null}},[]);
