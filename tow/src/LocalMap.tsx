...
export function LocalMap({caseId,dispatchId,dispatchStatus}:{caseId?:string;dispatchId?:string;dispatchStatus?:string}){
  ...
  const mapNode=useRef<HTMLDivElement|null>(null),mapRef=useRef<any>(null),markers=useRef<any[]>([]),lastPush=useRef(0),lastRouteAt=useRef(0),lastRouted=useRef<Point|null>(null),lastTarget=useRef('');
  const assigned=Boolean(caseId&&dispatchId);
  const mode=new Date().getHours()>=7&&new Date().getHours()<19?'day':'night';
  ...
  async function getRoute(from:Point,to:Point,force:boolean){const key=`${to.lng.toFixed(5)},${to.lat.toFixed(5)}`,changed=key!==lastTarget.current,moved=lastRouted.current?distanceM(from,lastRouted.current):Infinity;if(!force&&!changed&&Date.now()-lastRouteAt.current<10000&&moved<45)return;lastTarget.current=key;lastRouteAt.current=Date.now();lastRouted.current={...from};try{const r=await fetch(`${BASE}/api/local/route?from=${encodeURIComponent(`${from.lng},${from.lat}`)}&to=${encodeURIComponent(`${to.lng},${to.lat}`)}`,{cache:'no-store'});const d=await r.json() as RoutePayload;if(!r.ok||!d.route?.geometry?.coordinates?.length)throw new Error();setRoute(d.route);setError('')}catch{setError('ROVIQ Local road route is temporarily unavailable.')}}

  function drawRoute(r:Route|null){const map=mapRef.current;if(!map||!map.loaded?.())return;const coords=r?.geometry?.coordinates??[];try{const src=map.getSource('rq-route');const data={type:'Feature',properties:{},geometry:{type:'LineString',coordinates:coords}};if(src)src.setData(data);else if(coords.length)map.addSource('rq-route',{type:'geojson',data});if(coords.length&&!map.getLayer('rq-route-casing'))map.addLayer({id:'rq-route-casing',type:'line',source:'rq-route',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':'#07151a','line-width':10,'line-opacity':.82}});if(coords.length&&!map.getLayer('rq-route-core'))map.addLayer({id:'rq-route-core',type:'line',source:'rq-route',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':'#55dfce','line-width':5.2,'line-opacity':.96}})}catch{}}

  async function render(force:boolean){const map=mapRef.current;if(!map||!map.loaded?.())return;const tow=live??point(spatial?.transport_location),p=pickup(),d=destination(),t=target();markers.current.forEach(m=>m.remove?.());markers.current=[];if(p)markers.current.push(new window.maplibregl.Marker({element:endpointMarker('pickup'),anchor:'center'}).setLngLat([p.lng,p.lat]).addTo(map));if(d)markers.current.push(new window.maplibregl.Marker({element:endpointMarker('destination'),anchor:'center'}).setLngLat([d.lng,d.lat]).addTo(map));if(tow)markers.current.push(new window.maplibregl.Marker({element:towPuck(live?.heading??null),anchor:'center',rotationAlignment:'map'}).setLngLat([tow.lng,tow.lat]).addTo(map));if(tow&&t)await getRoute(tow,t,force);if(tow&&following)map.easeTo({center:[tow.lng,tow.lat],zoom:Math.max(map.getZoom(),15.2),pitch:assigned?50:28,bearing:live?.heading??map.getBearing(),duration:550,essential:true});else if(force){const pts=[p,d,tow].filter(Boolean) as Point[];if(pts.length>1){const b=new window.maplibregl.LngLatBounds();pts.forEach(x=>b.extend([x.lng,x.lat]));map.fitBounds(b,{padding:{top:160,bottom:220,left:42,right:42},maxZoom:15.2,pitch:assigned?50:28,duration:800,essential:true})}}}
