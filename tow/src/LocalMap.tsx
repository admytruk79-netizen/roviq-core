...
type Route={geometry?:{coordinates?:number[][]};distance?:number;duration?:number};
...
export function LocalMap({caseId,dispatchId,dispatchStatus}:Props){
  const[spatial,setSpatial]=useState<Spatial|null>(null),[live,setLive]=useState<LivePoint|null>(null),[route,setRoute]=useState<Route|null>(null);
  const[tracking,setTracking]=useState<'starting'|'live'|'limited'>('starting'),[error,setError]=useState(''),[following,setFollowing]=useState(true);
  const mapNode=useRef<HTMLDivElement|null>(null),mapRef=useRef<maplibregl.Map|null>(null),markers=useRef<maplibregl.Marker[]>([]);
  ...
  const mode:'day'|'night'=document.documentElement.dataset.roviqMode==='day'?'day':document.documentElement.dataset.roviqMode==='night'?'night':(new Date().getHours()>=7&&new Date().getHours()<19?'day':'night');
  ...
  return <section className="map-panel dispatch-context"><div className="map-head"><div><span className="eyebrow map-eyebrow">ROVIQ Local · Tow</span><h2>{assigned?(goingToPickup?'Tow → pickup':'Vehicle → destination'):'Tow live map'}</h2><p>{assigned?'Dedicated Tow overlay on the shared ROVIQ Local map.':'Live GPS map available before assignment.'}</p></div><span className={`live-badge ${tracking}`}>{tracking==='live'?'● GPS LIVE':tracking==='starting'?'LOCATING…':'GPS LIMITED'}</span></div><div className={`live-map-wrap rq-local-${mode}`}><div ref={mapNode} className="live-map"/><div className="rq-brand rq-tow-brand"><strong>ROVIQ</strong><span>LOCAL · TOW</span></div>{assigned&&<section className="rq-nav-hud"><div className="rq-nav-kicker">ROVIQ DRIVE</div><div className="rq-nav-next">{goingToPickup?'Proceed to pickup':'Proceed to destination'}</div><div className="rq-nav-meta"><strong>{Number.isFinite(etaMinutes)?`${Math.max(1,Math.round(etaMinutes))} min`:'—'}</strong><span>{Number.isFinite(distanceMiles)?`${distanceMiles.toFixed(1)} mi`:'—'}</span><span>{live?.speed!=null?`${Math.max(0,Math.round(live.speed*2.23694))} mph`:'0 mph'}</span></div><button type="button" className="rq-nav-follow" onClick={recenter} aria-label="Follow live tow location">◎</button></section>}{!assigned&&<button type="button" className="rq-locate rq-map-locate" onClick={recenter} aria-label="Center on live tow location">◎</button>}{error&&<div className="map-warning">{error}</div>}</div>{assigned&&<div className="spatial-grid"><div><span>Pickup</span><strong>{label(spatial?.origin??spatial?.current_vehicle)}</strong></div><div><span>Destination</span><strong>{label(spatial?.destination)}</strong></div><div><span>Tow position</span><strong>{label(live??spatial?.transport_location)}</strong></div><div><span>Current leg</span><strong>{goingToPickup?'Tow → pickup':'Vehicle → destination'}</strong></div></div>}</section>;
}
